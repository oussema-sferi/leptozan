...
function forceVid1Param() {
    let buyButtons = document.querySelectorAll('a[href*="buygoods.com"]');

    buyButtons.forEach(function(button) {
        var href = button.getAttribute('href');

        // Remove existing vid1 if present
        href = href.replace(/([?&])vid1=[^&]*(&)?/, function(_, sep, trailing) {
            return trailing ? sep : '';
        });
        ...
        // Append vid1=vsl01a cleanly
        let separator = href.includes('?') ? '&' : '?';
        href += separator + 'vid1=vsl01a';
